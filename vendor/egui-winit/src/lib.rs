        ...
        if let Some(active_key) = logical_key.or(physical_key) {
            if pressed {
                if is_cut_command(self.egui_input.modifiers, active_key) {
                    ...
                    // Keep the request even when the clipboard contains an image
                    // rather than text. Applications can handle their own image
                    // clipboard here; text widgets safely ignore an empty paste.
