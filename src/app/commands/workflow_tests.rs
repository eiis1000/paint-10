...
fn enter_custom_size(app: &mut PaintApp, ctx: &Context, value: &str, settle_popup: bool) {
    ...
    keys(app, ctx, &[Key::C]);
    settle(app, ctx);
    frame(app, ctx, vec![key(Key::A, Modifiers::CTRL)]);
...
fn custom_width_shortcuts_preserve_the_full_supported_range() {
    ...
    for (value, increased) in [(137, 138), (500, 500)] {
        enter_custom_size(&mut app, &ctx, &value.to_string(), true);
...
fn immediate_custom_size_keytip_overrides_pending_popup_focus() {
    ...
    let mut app = PaintApp::new_with_context(&ctx, false);
    settle(&mut app, &ctx);
    enter_custom_size(&mut app, &ctx, "137", false);
...
fn custom_rotation_cancel_preserves_the_draft_and_apply_targets_only_that_shape() {
    ...
    settle(&mut app, &ctx);
    frame(&mut app, &ctx, vec![key(Key::A, Modifiers::CTRL)]);
